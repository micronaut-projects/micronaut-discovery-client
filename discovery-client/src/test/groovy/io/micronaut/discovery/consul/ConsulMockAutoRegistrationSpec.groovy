...
class ConsulMockAutoRegistrationSpec extends Specification {
    ...
    void "test that a service can be registered with tags"() {
        ...
        then:
        conditions.eventually {
            List<ConsulHealthEntry> entry = Flux.from(client.getHealthyServices(serviceName)).blockFirst()
            entry.size() == 1
    ...
    void "test that a asl token can be configured"() {
        ...
        then:
        conditions.eventually {
            List<ConsulHealthEntry> entry = Flux.from(consulClient.getHealthyServices(serviceName)).blockFirst()
            entry.size() == 1
        }
