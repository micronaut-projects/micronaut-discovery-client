...
Supported importer query parameters include the common resiliency settings (`retry-attempts`, `retry-delay`, `read-timeout`, `connect-timeout`) plus Consul-specific options such as `format`, `dc`, `acl-token`, and `fail-fast`.
